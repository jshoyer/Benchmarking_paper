...
lofreq.df<-mutate(lofreq.df,mutation=paste0(CHROM,"_",REF,POS,ALT))
lofreq.df<-mutate(lofreq.df,category=mutation %in% true_snv$mutant,Id=ID,chr=CHROM,pos=POS,freq.var=AF)
lofreq.df$category[lofreq.df$mutation %in% wt_mut]<-"wt"
lofreq.df$category[lofreq.df$mutation %in% mfe.df$mutation]<-"mfe"
lofreq.df<-ddply(lofreq.df,~chr,primer.cut)

lofreq.df$p.val<-as.numeric(lofreq.df$QUAL) # to make the roc based on the quality scores given by lofreq
lofreq.df<-mutate(lofreq.df, gc = sub(pat,"\\1",Id),
       exp.freq = sub(pat,"\\2",Id)
)
