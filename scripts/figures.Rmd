...
pdf("~/Box Sync/Lab_work/Talks/2016_812/figures/roc.ex.pdf")
plot.ex(0.35,0.95,roc.df,example)
dev.off()
...
mapq_phred.normal<-ggplot(subset(sig.df,category %in% c(T,F) & gc==5),aes(x=Phred,y=MapQ,color=category))+geom_point(alpha=1)+geom_hline(yintercept=20)+geom_vline(xintercept=30)+scale_x_continuous(limits=c(30,40))+scale_color_hue(name="",labels=c("False posivite","True positive"))
...
mapq_phred.stingy<-ggplot(subset(sig.df,category %in% c(T,F) & gc==5),aes(x=Phred,y=MapQ,color=category))+geom_point(alpha=1)+scale_x_continuous(limits=c(30,40))+geom_hline(yintercept=30)+geom_vline(xintercept=35)+scale_color_hue(name="",labels=c("False positive","True positive"))
