...
plot.ex<-function(spec1,spec2,roc.df,distribution){ 
 ...
 points.p<-ggplot(distribution,aes(x=1,y=test,fill=Condition))+geom_dotplot(binaxis = "y", stackdir = "center",dotsize = 1.5)+scale_fill_manual(values=c("white","black"),name="",breaks=c(T,F),labels=c("True Positive","False Positive"))+xlab("")+ylab("Log(p value)")+theme(axis.ticks = element_blank(), axis.text.x = element_blank(),axis.title.y = element_text(vjust=1.2),legend.position=c(0.9,0.65),legend.background = element_rect(fill = alpha("white",0)))+xlab("")
...
miseq.sense.63<-m.roc.table$Sensitiviy[m.roc.table$Frequency=="0.63%"]
miseq.sense.16<-m.roc.table$Sensitiviy[m.roc.table$Frequency=="0.016%"]
...
mapq_phred.normal<-ggplot(subset(sig.df,category %in% c(T,F) & gc==5),aes(x=Phred,y=MapQ,color=category))+geom_point(size=1)+geom_hline(yintercept=20,linetype=2)+geom_vline(xintercept=30,linetype=2)+scale_x_continuous(limits=c(30,40))+scale_color_manual(name="",labels=c("False posivite","True positive"),values=c("#787878ff","#08519cff"))+ylab("Mean mapping quality")+xlab("Mean base qualtiy (Phred) ")+theme(legend.position="top") 
...
read.pos<-ggplot(subset(sig.cut.df,category %in% c(T,F) & gc==5),aes(x=Read_pos,fill=category))+geom_histogram(position='dodge')+scale_fill_manual(name="",labels=c("False positive","True positive"),values=c("#787878ff","#08519cff"))+xlab("Average position on read")+ylab("Number of variants")+theme(legend.position="top")
# read.pos
...
pat<-"([0-9]+)_([0-9]+)"
mutate(dup.df, exp.freq = sub(pat,"\\1",Id),
...
Richness
Inverse Simpsons D ( without replacement)
Shannon Entropy
For these I'm assuming all varaints are independent
...
get_data<-function(x,setting){
  ...
  out<-data.frame(Richness=round(rich(x),digits = 0), Entropy=format(H(x),scientific = T,digits = 3),Manhattan=round(euc(x,subset(perfect,freq.var==exp.freq)),digits=3)) #"1/D"=D(x),
  rownames(out)<-setting
  return(t(out))
}
tabulate<-function(input,freq){
get_data(subset(perfect,freq.var==freq),"Expected")->perf
get_data(subset(sum.df,gc==input & exp.freq==freq & p.val<0.01 & category %in% c(T,F)),"DeepSNV")->deepSNV
get_data(subset(lofreq.df,gc==input & exp.freq==freq & category %in% c(T,F)),"Lofreq")->lo
get_data(subset(cut.read.df.all,gc==input & exp.freq==freq & p.val<0.01 & category %in% c(T,F) ),"DeepSNV Final")->deepSNV.f
knitr::kable(cbind(perf,deepSNV,lo,deepSNV.f))
}
# tabulate.4<-function(input,freq){
# get_data(subset(perfect,freq.var==freq),"Perfect")->perf
# get_data(subset(sum.df,gc==input & exp.freq==freq),"DeepSNV")->deepSNV
# get_data(subset(lofreq.df,gc==input & exp.freq==freq),"Lofreq")->lo
# get_data(subset(cut.read.sig.df,gc==input & exp.freq==freq),"DeepSNV Final")->deepSNV.f
# get_data(subset(dups,exp.freq==freq),"DeepSNV duplicates")->deepSNV.d
# 
# knitr::kable(cbind(perf,deepSNV,lo,deepSNV.f,deepSNV.d),digits = 2)
# }
