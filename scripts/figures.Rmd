...
get_data<-function(x,setting){
  ...
  out<-data.frame(Richness=round(rich(x),digits = 0), Entropy=format(H(x),scientific = T,digits = 3),"1/D"=D(x),Manhattan=round(euc(x,subset(perfect,freq.var==exp.freq)),digits=3))
  rownames(out)<-setting
  return(t(out))
}
tabulate<-function(input,freq){
get_data(subset(perfect,freq.var==freq),"Perfect")->perf
get_data(subset(sum.df,gc==input & exp.freq==freq & p.val<0.01),"DeepSNV")->deepSNV
get_data(subset(lofreq.df,gc==input & exp.freq==freq),"Lofreq")->lo
get_data(subset(cut.read.df.all,gc==input & exp.freq==freq & p.val<0.01),"DeepSNV Final")->deepSNV.f
