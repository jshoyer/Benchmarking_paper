...
> make sure the possible fp accounts for the fact that we aren't looking at the priming regions for both Miseq and Hiseq
## Figure 1 
...
ggdraw() +
  ...
  draw_plot(tbl, 0.45, 0, 0.53, 0.47) +
  draw_plot_label(c("A", "B", "C"), c(0.05, 0.4, 0.42), c(0.95, 0.95, 0.46), size = 15)
...
lofreq.table<-subset(lofreq.table,gc==5,select=c(exp.freq,TP,FP))
...
dups.roc.p<-ggplot(dups.roc,aes(x=1-adj.specificity,y=adj.sensitivity))+geom_step(aes(color=exp.freq),size=2)+scale_color_manual(values=rhg_cols,name="Frequency",labels=c("5.0%","2.0%","1.0%","0.5%","0.2%"))+xlab(bquote("1-Specificity (x"~10^-3~")"))+ylab("Sensitivity")+guides(color=guide_legend(title="Frequency"))+scale_y_continuous(limits=c(0,1))+scale_x_continuous(limits=c(0,0.001),breaks=c(0,0.0005,0.001),labels=c(0,0.5,1))+theme(legend.position="none")
