...
lofreq.df<-read.csv("../data/process/2015-6-23/Lofreq/lofreq_csv/all.lofreq.csv",stringsAsFactors = F,comment.char="#")


lofreq.df<-rename(lofreq.df,c("CHROM"="chr",
                        "POS"="pos",
                        "ID"="Id",
                        "REF"="ref",
                        "ALT"="var",
...
lofreq.df<-mutate(lofreq.df,mutation=paste0(chr,"_",ref,pos,var))#,category=mutation %in% tp$mutant)->lofreq.df

#lofreq.df$Id[lofreq.df$Id=="3_03"]<-"3_02" # correct a naming error

WT<-subset(lofreq.df,grepl("WT",Id)) # Separate WT samples 
lofreq.df<-subset(lofreq.df,!(grepl("WT",Id))& Id!="Plasmid_control")



#lofreq.df<-mutate(lofreq.df,p.val=10^(QUAL/-10))

pat<-"([0-9]+)_([0-9]+)"
mutate(lofreq.df, gc = sub(pat,"\\1",Id),
       exp.freq = sub(pat,"\\2",Id)
)-> lofreq.df
mutate(lofreq.df,exp.freq=ifelse(test = grepl("0",exp.freq),yes=as.numeric(exp.freq)/1000,no = as.numeric(exp.freq)/100))->lofreq.df

# TF column
lofreq.df<-mutate(lofreq.df,category=mutation %in% true_snv$mutant) # add Column for True and false variants
...
lofreq.table<-subset(lofreq.table,gc==5,select=-c(gc,Id)) # don't need the 0.1% sample
...
#dup.df$category[dup.df$mutation %in% wt_mut]<-"wt" There are no wt mutants see wt_mut. For some reason this was causing an error.
dup.df$category[dup.df$mutation %in% mfe.df$mutation]<-"mfe"
