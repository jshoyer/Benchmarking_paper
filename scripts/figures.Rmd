...
euc<-function(x,ref){
  x$Id="test"
  ref$Id="ref"
  all<-rbind(subset(x,select=c(mutation,freq.var,Id)),ref)
...
get_data<-function(x,setting){
  if(length(unique(x$freq.var))==1){
    exp.freq=unique(x$freq.var)
  }else{
  exp.freq=unique(x$exp.freq)
  }
  out<-data.frame(Richness=rich(x), Entropy=H(x),Simpson=D(x))#,dist=euc(x,subset(perfect,freq.var==exp.freq))[1])
...
knitr::kable(cbind(perf,deepSNV,lo,deepSNV.f),digits = 2)
}
