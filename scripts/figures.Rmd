...
adjust.coords<-function(roc.df,sum.df){ # adjust the sensitivity and specificity called by pROC  
  samp<-roc.df$samp[1]
...
roc.df<-roc_df.one(roc.ls,0.001)
roc.df.adj<-ddply(roc.df,~samp,adjust.coords,sum.df)
roc.df.adj<-roc.df.adj[order(roc.df.adj$exp.freq,decreasing = T),]


knitr::kable(subset(roc.df.adj,select=c(exp.freq,adj.sensitivity,TP,adj.specificity,FP)))
