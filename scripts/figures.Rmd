...
Figure 2. Initial DeepSNY accuracy. A) RT-PCR amplified WSN33 genomes were diluted into PR8 at decreasing frequencies and sequenced on an Illumina Miseq. B) An ROC showing our ability to identify WSN33 SNV relative to PR8 at the indicated frequencies. C) A table of the DeepSNV accuracy at a p value threshold of 0.01.
...
Figure 3. DeepSNV accuracy under conditions that more accurately mimic those found in patient derived samples. A) 20 known single point mutants were diluted into wild type at decreasing frequencies. These were diluted further into viral media to match the genomes concentration found in patent samples (10$^5$-10$^3$ genomes/$\mu$l). B) An ROC indicating our ability to accurately identify the 20 known variants at 10 $^5$ genomes/$\mu$l. C) A table of the accuracy of our variant calling for the 10 $^5$ genomes/$\mu$l samples at a p value threshold of 0.01.
...
x$adj.sensitivity[x$frequency=="1.0%"]<-x$adj.sensitivity[x$frequency=="1.0%"]-0.01
x$adj.sensitivity[x$frequency=="2.0%"]<-x$adj.sensitivity[x$frequency=="2.0%"]+0.08
...
Figure 4. Investigating the quality of variants in the the 10 $^5$ genomes/$\mu$l samples. A) All called variants from the 10 $^5$ genomes/$\mu$l samples with p<0.01 stratified by the mean mapping quality of the reads containing the variants and the mean Phred scores of the variant bases. Dashed lines indicate the common cutoffs of 20 and 30 for mapping quality and Phred respectively. B) A histogram of all called variants from the 10 $^5$ genomes/$\mu$l samples with p<0.01 binned by the average position of the variant relative to the Hiseq reads on which it is found. C) An ROC of the accuracy of the variant caller when the following quality cut offs are applied : mean mapping quality of 30, mean Phred of 35, and an average read position between 32 and 94 (the middle 50% of the read). D) A table summarizing the ROC in C at a p value threshold of 0.01.
...
Figure 5. The accuracy of the common variant caller Lofreq. The variant caller Lofreq was applied to the 10 $^5$ genomes/$\mu$l samples. A) An ROC of the accuracy of Lofreq using standard parameters. The specificity of the caller was scaled to account for the same number of tests as the DeepSNV algorithm. Lofreq provides qualtiy scores instead of p values. These were used to set cutoffs for the ROC. B) A table summarizing the accuracy of the ROC A.
...
Figure 7. The accuracy of DeepSNV at lower input levels. A) An ROC depicting the accuracy of our method when applied to the 10 $^4$ genomes/$\mu$l samples. B) A table summarizing the ROC in A at a p value threshold of 0.01. C) An ROC depicting the accuracy of our method when applied to the 10 $^3$ genomes/$\mu$l samples. B) A table summarizing the ROC in C at a p value threshold of 0.01.
...
Figure 8. The accucary of DeepSNV at lower input levels when run in duplicate. A subset of the 10 $^4$ genomes/$\mu$l samples were processed in duplicate to control for RT-PCR errors. A) The accuracy of our varant calling method at 10 $^4$ genomes/$\mu$l is greatly enhanced by requiring putative variants to be found in duplicate RT-PCR reactions. B) A table summarizing the ROC in C at a p value threshold of 0.01.