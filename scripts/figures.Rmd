...
plot.ex<-function(spec1,spec2,roc.df,distribution){ 
 ...
 points.p<-ggplot(distribution,aes(x=1,y=test,fill=Condition))+geom_dotplot(binaxis = "y", stackdir = "center")+scale_fill_manual(values=c("white","black"),name="",breaks=c(T,F),labels=c("True Positive","False Positive"))+xlab("")+ylab("Log(p value)")+theme(axis.ticks = element_blank(), axis.text.x = element_blank(),axis.title.y = element_text(vjust=1.2))+xlab("")
 ...
 plot_grid(points.p, roc.p, labels=c("A", "B"), ncol = 2, nrow = 1)
...
miseq.roc.p<-ggplot(m.roc.df,aes(x=1-adj.specificity,y=adj.sensitivity))+geom_step(aes(color=exp.freq),size=2)+scale_color_manual(values=rhg_cols)+xlab(bquote("1-Specificity (x"~10^-3~")"))+ylab("Sensitivity")+guides(color=guide_legend(title="Frequency"))+scale_y_continuous(limits=c(0,1))+scale_x_continuous(limits=c(0,0.005),breaks=c(0,0.001,0.002,0.003,0.004,0.005),labels=c(0:5))+theme(legend.position=c(0.8,.5))
...
m.roc.table$Sensitivity<-round(m.roc.table$Sensitivity,digits=3)
m.roc.table$Specificity<-round(m.roc.table$Specificity,digits=3)

tbl<-tableGrob(m.roc.table,rows=NULL,theme=ttheme_minimal()) # Make table
...
# Figure 2
ggdraw() +
  draw_plot(miseq.setup, 0, .5, .5, .5) +
  draw_plot(miseq.roc.p, .5, .5, .5, .5) +
  draw_plot(tbl, 0, 0, 1, .5) +
...
# pdf("~/Box Sync/Lab_work/Talks/2016_812/figures/hiseq.roc.pdf")
# Hiseq.roc
# dev.off()
...
h.table.df$Sensitivity<-round(h.table.df$Sensitivity,digits=3)
h.table.df$Specificity<-round(h.table.df$Specificity,digits=3)

tbl<-tableGrob(h.table.df,rows=NULL,theme=ttheme_minimal()) # Make table
...
ggdraw() +
  draw_plot(hiseq.setup, 0, .5, .5, .5) +
  draw_plot(Hiseq.roc, .5, .5, .5, .5) +
  draw_plot(tbl, 0, 0, 1, .5) +
  draw_plot_label(c("A", "B", "C"), c(0, 0.5, 0), c(1, 1, 0.5), size = 15)
...
mapq_phred.normal<-ggplot(subset(sig.df,category %in% c(T,F) & gc==5),aes(x=Phred,y=MapQ,color=category))+geom_point(alpha=1)+geom_hline(yintercept=20,linetype=2)+geom_vline(xintercept=30,linetype=2)+scale_x_continuous(limits=c(30,40))+scale_color_hue(name="",labels=c("False posivite","True positive"))+ylab("Mean mapping quality")+xlab("Mean base qualtiy (Phred) ")
...
read.pos<-ggplot(subset(sig.cut.df,category %in% c(T,F) & gc==5),aes(x=Read_pos,fill=category))+geom_histogram(position='dodge')+scale_fill_hue(name="",labels=c("False positive","True positive"))+xlab("Average position on read")+ylab("Number of variants")+theme(legend.position=c(.5,.7))
# read.pos
...
# read.roc
# 
# pdf("~/Box Sync/Lab_work/Talks/2016_812/figures/read.roc.pdf")
# read.roc
# dev.off()
...
h.cut.read.table.5$Sensitivity<-round(h.cut.read.table.5$Sensitivity,digits=3)
h.cut.read.table.5$Specificity<-round(h.cut.read.table.5$Specificity,digits=3)

tbl<-tableGrob(h.cut.read.table.5,rows=NULL,theme=ttheme_minimal()) # Make table


ggdraw() +
  draw_plot(mapq_phred.normal, 0, .5, .5, .5) +
  draw_plot(read.pos, .5, .5, .5, .5) +
  draw_plot(read.roc, 0, 0, 0.5, .5) +
  draw_plot(tbl, 0.5, 0, 0.5, .5)+
...
tbl<-tableGrob(lofreq.table,rows=NULL,theme=ttheme_minimal()) # Make table
...
ac_freq<-ac_freq+geom_dotplot(binaxis = "y", stackdir = "center",dotsize=1)+scale_color_manual(values=rhg_cols)
...
pdf("~/Box Sync/Lab_work/Talks/2016_812/figures/freq.pdf")
ac_freq
dev.off()
...
h.cut.read.table.4$Sensitivity<-round(h.cut.read.table.4$Sensitivity,digits=3)
h.cut.read.table.4$Specificity<-round(h.cut.read.table.4$Specificity,digits=3)

tbl.4<-tableGrob(h.cut.read.table.4,rows=NULL,theme=ttheme_minimal()) # Make table
...
h.cut.read.table.3$Sensitivity<-round(h.cut.read.table.3$Sensitivity,digits=3)
h.cut.read.table.3$Specificity<-round(h.cut.read.table.3$Specificity,digits=3)

tbl.3<-tableGrob(h.cut.read.table.3,rows=NULL,theme=ttheme_minimal()) # Make table


ggdraw() +
  draw_plot(roc.4, 0, .5, .5, .5) +
  draw_plot(tbl.4, .5, .5, .5, .5) +
  draw_plot(roc.3, 0, 0, 0.5, .5) +
  draw_plot(tbl.3, 0.5, 0, 0.5, .5)+
  draw_plot_label(c("A", "B", "C","D"), c(0, 0.5, 0,0.5), c(1, 1, 0.5,0.5), size = 15)
...
dups.table$Sensitivity<-round(dups.table$Sensitivity,digits=3)
dups.table$Specificity<-round(dups.table$Specificity,digits=3)

tbl<-tableGrob(dups.table,rows=NULL,theme=ttheme_minimal()) # Make table

ggdraw() +
  draw_plot(dups.roc.p, 0, .5, 0.5, .5) +
  draw_plot(tbl, 0.5, 0.5, 0.5, .5) +
  draw_plot_label(c("A", "B"), c(0, 0.5), c(1, 1), size = 15)
...
### Concequences for diversity measurements
